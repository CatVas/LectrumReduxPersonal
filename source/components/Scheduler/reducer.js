...
export default handleActions({
    ...
    [types.DELETE_TASK_SUCCESS]: (state, { payload }) => state
        .update(
            'tasks',
            (tasks) => tasks.filter(
                (task) => task.get('id') !== payload.taskId
            ),
        )
        .set('loading', false),
    ...
    [types.TASK_INPUT_CHANGE]: (state, { payload }) => state
        .setIn(['taskInput', 'value'], payload.value),

    [types.TOGGLE_TASK_EDIT]: (state, { payload }) => {
        const isEdited = state.getIn(['taskEdited', 'id']) === payload.id;

        return state
            .setIn(['taskEdited', 'id'], isEdited ? '' : payload.id)
            .setIn(['taskEdited', 'message'], isEdited ? '' : payload.message);
    },
